...
export async function getStrategies(): Promise<Strategy[]> {
    ...
    const snap = await getDocs(q);
    return snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Strategy));
}

export async function addStrategy(data: Omit<Strategy, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
...
export async function seedStrategies(): Promise<void> {
    try {
        const existing = await getStrategies();
        if (existing.length > 0) {
            console.log(`Strategies already seeded (${existing.length} found).`);
            return;
        }

        const initialStrategy: Omit<Strategy, 'id' | 'createdAt' | 'updatedAt'> = {
...
Expected ROI: First agent project pays back all setup costs 5x over.`,
        ...
        console.log('[Seed] Seeding initial AI Audit + AI Agents strategy...');
        await addStrategy(initialStrategy);
        console.log('[Seed] Initial strategy seeded successfully.');
    } catch (error) {
