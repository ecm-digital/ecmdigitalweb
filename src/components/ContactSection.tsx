...
function ContactForm({ T }: { T: (key: string) => string }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // 3. Submit directly to HubSpot public Forms API
            ...
            const hsPayload = {
                fields: [
                    ...
                    { name: 'message', value: `Selected Service: ${data.service || 'None'}\nMessage: ${data.message || ''}` }
                ],
                context: {
                    hutk: getHubSpotCookie(),
                    pageUri: typeof window !== 'undefined' ? window.location.href : 'https://ecmdigital.pl/kontakt',
                    pageName: typeof document !== 'undefined' ? document.title : 'Kontakt'
                }
            };
            ...
            .then(res => res.json())
            .then(resData => console.log('HubSpot client-side response:', resData))
            ...
            <div className="form-row">
                ...
                <div className="form-group">
                    ...
                    <select id="contact-service" name="service">
                        ...
                        <option value="websites_leads">{T('contact.form.service.opt1')}</option>
                        <option value="ai_followup">{T('contact.form.service.opt2')}</option>
                        <option value="automation_crm">{T('contact.form.service.opt3')}</option>
                        <option value="audit_training">{T('contact.form.service.opt4')}</option>
                    </select>
                </div>
            </div>
            <div className="form-group">
