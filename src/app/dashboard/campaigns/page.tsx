            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        ...
                        <p className="text-[10px] text-white/30 font-black uppercase tracking-[0.2em] mt-2">Monitoruj wyniki działań marketingowych w czasie rzeczywistym</p>
                    </div>
                </div>

                {/* Dashboard stats row */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                    <div className="bg-[#050508]/40 backdrop-blur-3xl border border-white/5 rounded-[40px] p-10 relative overflow-hidden group shadow-2xl">
                        ...
                        <span className="text-[9px] text-white/20 uppercase tracking-[0.2em] font-black block mb-6 italic">Aktywny Budżet</span>
                        <div className="text-4xl font-black font-space-grotesk tracking-tighter italic text-white">{totalBudget.toLocaleString('pl-PL')} <span className="text-sm opacity-20 non-italic">PLN</span></div>
                        <div className="mt-6 flex items-center gap-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-brand-accent animate-pulse shadow-[0_0_10px_#8b5cf6]"></div>
                            <span className="text-[9px] text-white/30 font-black uppercase tracking-widest">Zakumulowany kapitał</span>
                        </div>
                    </div>
                    <div className="bg-[#050508]/40 backdrop-blur-3xl border border-white/5 rounded-[40px] p-10 relative overflow-hidden group shadow-2xl">
                        ...
                        <span className="text-[9px] text-white/20 uppercase tracking-[0.2em] font-black block mb-6 italic">Realne Wydatki</span>
                        <div className="text-4xl font-black font-space-grotesk tracking-tighter italic text-brand-accent">{totalSpent.toLocaleString('pl-PL')} <span className="text-sm opacity-20 non-italic text-white">PLN</span></div>
                        <div className="mt-6 flex items-center gap-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
                            <span className="text-[9px] text-white/30 font-black uppercase tracking-widest">Koszty mediowe netto</span>
                        </div>
                    </div>
                    <div className="bg-[#050508]/40 backdrop-blur-3xl border border-white/5 rounded-[40px] p-10 relative overflow-hidden group shadow-2xl">
                        ...
                        <span className="text-[9px] text-white/20 uppercase tracking-[0.2em] font-black block mb-6 italic">Wynik: Konwersje</span>
                        ...
                        <div className="mt-6 flex items-center gap-3">
                            ...
                            <span className="text-[9px] text-white/30 font-black uppercase tracking-widest">Leady / Sprzedaż / Akcje</span>
                        </div>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="py-32 bg-[#050508]/40 backdrop-blur-3xl border border-white/5 border-dashed rounded-[48px] text-center group">
                        ...
                        <h3 className="text-2xl font-black font-space-grotesk italic uppercase tracking-tighter mb-2">Brak aktywnych kampanii</h3>
                        <p className="text-white/30 text-sm max-w-md mx-auto px-6 font-medium">Obecnie nie prowadzimy żadnych aktywnych kampanii reklamowych dla Twojej marki.</p>
                        <button className="mt-12 px-10 py-5 bg-white/5 border border-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-brand-accent transition-all">
                            Zapytaj o Strategię Ads
                        </button>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 pb-12">
                        {campaigns.map(c => {
                            ...
                            return (
                                <div key={c.id} className="bg-[#050508]/40 backdrop-blur-3xl border border-white/5 rounded-[48px] p-10 flex flex-col hover:bg-white/[0.05] transition-all group relative overflow-hidden shadow-2xl">
                                    ...
                                    <div className="flex justify-between items-start mb-10 relative z-10">
                                        <div className="flex-1 min-w-0 pr-4">
                                            <div className="text-[9px] font-black text-brand-accent uppercase tracking-[0.2em] mb-3">{c.platform}</div>
                                        ...
                                        <span className={`text-[9px] px-3 py-1.5 rounded-full font-black uppercase tracking-widest border transition-colors ${statusColors[c.status] || ''}`}>
                                            {c.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-6 p-8 bg-black/40 rounded-[32px] border border-white/5 mb-10 relative z-10 shadow-inner">
                                        <div className="text-center">
                                            ...
                                            <div className="text-[9px] text-white/20 font-black uppercase tracking-widest mt-2 px-1">Kliknięcia</div>
                                        </div>
                                        <div className="text-center border-x border-white/5">
                                            <div className="text-2xl font-black font-space-grotesk italic text-emerald-400">{c.conversions}</div>
                                            <div className="text-[9px] text-white/20 font-black uppercase tracking-widest mt-2 px-1">Konwersje</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-2xl font-black font-space-grotesk italic text-brand-accent">{c.cpa.toLocaleString('pl-PL')} <span className="text-[10px] non-italic opacity-40">PLN</span></div>
                                            <div className="text-[9px] text-white/20 font-black uppercase tracking-widest mt-2 px-1">Koszt CPA</div>
                                        </div>
                                    </div>

                                    <div className="space-y-4 mb-10 relative z-10">
                                        <div className="flex justify-between items-end text-[10px] font-black uppercase tracking-widest italic">
                                            <span className="text-white/30">Wykorzystanie Budżetu</span>
                                        ...
                                        <div className="flex justify-between text-[9px] font-black uppercase tracking-[0.1em] text-white/20">
                                            <span>Wydane: {c.spent.toLocaleString()} PLN</span>
                                            <span>Limit: {c.budget.toLocaleString()} PLN</span>
                                        </div>
                                    </div>

                                    <div className="flex items-center justify-between mt-auto pt-8 border-t border-white/5 relative z-10">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] text-white/20 font-black uppercase tracking-widest italic">Okres Działań</span>
                                            <span className="text-[10px] font-bold text-white/40 mt-1 uppercase tracking-tight">{c.startDate} · {c.endDate}</span>
                                        </div>
                ...
                <div className="p-10 bg-gradient-to-br from-brand-accent/30 via-purple-600/30 to-blue-600/30 border border-white/20 rounded-[48px] backdrop-blur-3xl relative overflow-hidden group shadow-2xl">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                        ...
                        <div>
                            <h4 className="text-xl font-black font-space-grotesk uppercase italic mb-3 tracking-tighter">Analiza Skalowalności</h4>
                            <p className="text-sm text-white/80 leading-relaxed font-medium max-w-2xl">
                                Nasz system AI stale monitoruje Twoje kampanie pod kątem "Budget Capping". Jeśli widzimy potencjał na niższy CPA przy wyższym budżecie, Twój opiekun prześle odpowiednią sugestię w sekcji **Oferty**.
                            </p>
                        </div>
                    </div>
                </div>
