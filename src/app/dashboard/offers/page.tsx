                    ...
                    <div className={`${selectedOffer ? 'xl:w-[55%]' : 'w-full'} transition-all duration-700`}>
                        {loading ? (
                            ...
                            <div className="py-32 bg-[#050508]/40 backdrop-blur-3xl border border-white/5 border-dashed rounded-[48px] text-center group">
                                ...
                                <p className="text-white/30 text-sm max-w-md mx-auto px-6 font-medium">Skontaktuj się ze swoim opiekunem, aby przygotować spersonalizowaną ofertę dla Twojego biznesu.</p>
                            ...
                            <div className="bg-[#050508]/40 backdrop-blur-3xl border border-white/5 p-10 md:p-12 rounded-[48px] shadow-2xl relative overflow-hidden flex flex-col h-full">
                                <div className="absolute top-0 right-0 w-64 h-64 bg-brand-accent/5 blur-[100px] -mr-32 -mt-32 rounded-full"></div>

                                <div className="flex justify-between items-start mb-10 relative z-10">
                                    <div>
                                        <div className="text-[9px] font-black text-brand-accent uppercase tracking-[0.2em] mb-3">Oferta nr {selectedOffer.id.slice(-6).toUpperCase()}</div>
                                    ...
                                    <span className={`text-[9px] px-4 py-2 rounded-full font-black uppercase tracking-widest border transition-colors ${statusColors[selectedOffer.status] || ''}`}>
                                    ...
                                    {selectedOffer.items.map((item, i) => (
                                        <div key={i} className="flex justify-between items-center p-6 rounded-[32px] bg-white/5 border border-white/5">
                                            <div className="flex-1 pr-6">
                                                ...
                                                <div className="text-[11px] text-white/30 font-medium uppercase tracking-widest leading-relaxed">{item.description}</div>
                                            </div>
                                            <div className="text-2xl font-black font-space-grotesk text-white whitespace-nowrap">{item.price.toLocaleString('pl-PL')} <span className="text-xs text-white/20 non-italic">PLN</span></div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="mt-auto pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
                                    <div>
                                        <div className="text-[10px] uppercase tracking-widest text-white/30 font-black mb-2 italic">Całkowita wartość inwestycji</div>
                                    ...
                                    {selectedOffer.status === 'Wysłana' && (
                                        <div className="flex gap-4 w-full md:w-auto">
                                            <button
                                                ...
                                                className="flex-1 md:flex-none px-8 py-5 rounded-2xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-red-500/10 hover:text-red-400 hover:border-red-500/20 transition-all font-space-grotesk"
                                            >
                                                Odrzuć
                                            </button>
                                            <button
                                                onClick={() => handleAccept(selectedOffer.id)}
                                                className="flex-1 md:flex-none px-10 py-5 bg-brand-accent text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:shadow-[0_0_30px_rgba(139,92,246,0.3)] hover:scale-105 transition-all font-space-grotesk"
                                            >
                                                Akceptuję Ofertę
                                            </button>
                                        </div>
                                    ...
                                    {selectedOffer.status === 'Zaakceptowana' && (
                                        <div className="flex items-center gap-3 text-emerald-400 font-black text-[11px] uppercase tracking-widest bg-emerald-500/10 px-8 py-5 rounded-2xl border border-emerald-500/20">
                                ...
                                {selectedOffer.videoUrl && (
                                    <div className="mt-12 pt-10 border-t border-white/5 relative z-10">
                                        <div className="text-[10px] font-black text-brand-accent uppercase tracking-[0.2em] mb-6 italic">Wideo-omówienie strategii:</div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                {offers.map(o => (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start mb-10 relative z-10">
                                            <div>
                                                <div className="text-[9px] font-black text-brand-accent uppercase tracking-[0.2em] mb-3">Oferta nr {o.id.slice(-6).toUpperCase()}</div>
                                            ...
                                            <span className={`text-[9px] px-3 py-1.5 rounded-full font-black uppercase tracking-widest border transition-colors ${statusColors[o.status] || ''}`}>
                                                {o.status}
                                            </span>
                                        </div>
                                        ...
                                        <div className="space-y-4 mb-10 flex-1 relative z-10">
                                            {o.items.slice(0, 3).map((item, i) => (
                                                <div key={i} className="flex justify-between items-center p-5 rounded-[24px] bg-white/5 border border-white/5">
                                                    <div className="text-sm font-bold tracking-tight text-white/80">{item.service}</div>
                                                    <div className="text-sm font-black font-space-grotesk text-white/50">{item.price.toLocaleString('pl-PL')} PLN</div>
                                                </div>
                                            ))}
                                            {o.items.length > 3 && <div className="text-[9px] text-center font-black text-white/20 uppercase tracking-widest">+ {o.items.length - 3} więcej usług</div>}
                                        </div>

                                        <div className="pt-8 border-t border-white/5 flex justify-between items-center relative z-10">
                                            <div className="text-2xl font-black font-space-grotesk text-white italic">{o.totalPrice.toLocaleString('pl-PL')} <span className="text-[10px] text-white/30 non-italic">PLN</span></div>
                ...
                {!selectedOffer && (
                    <div className="p-10 bg-[#050508]/40 backdrop-blur-3xl border border-white/5 rounded-[48px] shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mb-6 italic">Wskazówki dla partnera</h4>
